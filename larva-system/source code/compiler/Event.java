...
//		}
	...
	{
		if (context.localVariables.containsKey(tokens.get(cnt).text))
		{
			Variable v = context.localVariables.get(tokens.get(cnt).text);
	...
	{	
		ArrayList<Token> list = Tokenizer.ending(cnt, ".", tokens);//.
		cnt += list.size() +1;//.
...
//		allWhere.addAll(recursiveWhereClauses(events, this, new ArrayList<Token>()));
		...
		throws ParseException
	{
		...
		{
			sb.append("Channel _c) : (call(* Channel.receive(..)) && target(_c) && (if (_c.equals(_cls_"+g.name+g.id+"."+ this.channelName +")))");
		...
		else
		{
			...
			{
				sb.append(") returning (");
				if (returned != null && variables.containsKey(returned.text))// && variables.get(returned.text).type != null)
					sb.append(variables.get(returned.text).getVariableType() + " " + variables.get(returned.text).getVariableName());
//				else
//				sb.append("*");
				sb.append(") : (");
			}
			else if (type == EventType.uponThrowing)
			{
				sb.append(") throwing (");
				if (returned != null && variables.containsKey(returned.text))// && variables.get(returned.text).type != null)
					sb.append(variables.get(returned.text).getVariableName());
				sb.append(") : (");
			}
		...
		for (Trigger t : events.values())
			if (t instanceof EventCollection)
		{
...
//			sb.append("\r\n_cls_inst."+v.name+"="+v.name+";");
			...
			for (Variable v : allOtherVars.values())
				...
				else if (this.type.equals(EventType.clock) || this.type.equals(EventType.clockCycle))
				{
					sb.append("\r\nsynchronized(_c){" +
